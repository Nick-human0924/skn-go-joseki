(function () {
  ...
  const state = { client: null, user: null, uploading: false, debounceId: 0, status: "未登录：数据只保存在当前浏览器" };
  ...
  function ensureUi() {
    ...
    if (!$("accountSyncDetails")) {
      ...
      details.innerHTML = `
        ...
        <div class="auth-panel" id="authPanel">
          ...
          <label class="field-row">邮箱<input id="authEmail" type="email" autocomplete="email" placeholder="parent@example.com" /></label>
          <label class="field-row">密码<input id="authPassword" type="password" autocomplete="current-password" placeholder="至少 6 位密码" /></label>
    ...
    if (!$("supabaseSyncStyle")) {
      ...
      style.textContent = `
        .auth-panel{display:grid;gap:10px}.auth-account{display:flex;align-items:baseline;justify-content:space-between;gap:8px}.auth-account span{font-weight:700}.auth-account small,.sync-status{color:var(--muted,#64748b)}.auth-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.signed-in-actions{display:none}.auth-panel.is-signed-in .signed-in-actions{display:grid}.auth-panel.is-signed-in #loginButton,.auth-panel.is-signed-in #signupButton{display:none}.sync-status{margin:0;font-size:12px;line-height:1.5}`;
  ...
  function readLocalStore() {
    ...
    try { return JSON.parse(raw); } catch (_error) { throw new Error("本地数据不是有效 JSON，已停止云同步"); }
  }
  ...
  async function uploadLocalStore(reason) {
    ...
    try {
      ...
      const { error } = await state.client.from(TABLE).upsert({ user_id: state.user.id, store }, { onConflict: "user_id" });
  ...
  async function pullOrSeedCloudStore() {
    ...
    const { data, error } = await state.client.from(TABLE).select("store, updated_at").eq("user_id", state.user.id).maybeSingle();
    if (error) { setStatus(error.message || "读取云端数据失败", true); return; }
    if (data?.store) {
  ...
  async function login() {
    ...
    if (!values.email || !values.password) { setStatus("请输入邮箱和密码", true); return; }
  ...
  async function signup() {
    if (!state.client) return;
    const values = getAuthValues();
    if (!values.email || values.password.length < 6) { setStatus("请输入邮箱和至少 6 位密码", true); return; }
    setStatus("正在注册...", false);
    const { data, error } = await state.client.auth.signUp(values);
    if (error) { setStatus(error.message || "注册失败", true); return; }
    if ($("authPassword")) $("authPassword").value = "";
    setStatus(data.session ? "注册成功，正在同步" : "注册成功，请按邮件提示确认后再登录", false);
  }
  ...
  async function initSupabase() {
    ...
    if (error) { setStatus(error.message || "读取登录状态失败", true); return; }
    state.user = data.session?.user || null;
    state.status = state.user ? "已登录，准备同步" : "未登录：数据只保存在当前浏览器";
    updateUi();
    if (state.user) await pullOrSeedCloudStore();
  }
